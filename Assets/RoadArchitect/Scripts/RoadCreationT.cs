    ...
    public static class RoadCreationT
    {
        ...
        public static void RoadJobPrelim(ref Road _road)
        {
            ...
            while (!isFinalEnd && spamcheck1 < spamcheckmax1)
            {
                ...
                //Debug.Log("while");
                //Debug.Log(i);
                //(new GameObject(tVect.ToString())).transform.position=tVect;
                //global::test2.test();
                //Vector3 myVect = new Vector3(tVect.x,tVect.y,tVect.z);
                //UnityMainThreadDispatcher.Instance().Enqueue(() => (new GameObject(myVect.ToString())).transform.position = myVect);
                //Profiler.EndSample();
